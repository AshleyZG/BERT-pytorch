...
def train():
    ...
    data_reader = DataReader(
        args.dataset, seq_len=args.seq_len, use_sub_token=args.use_sub_token, max_graph_num=args.max_graph_num)
    ...
    train_dataset = CustomBERTDataset(data_reader.graphs[:int(len(data_reader) * 0.8)], vocab, markdown_vocab, seq_len=args.seq_len,
                                      on_memory=args.on_memory, n_neg=args.duplicate, use_sub_token=args.use_sub_token, n_topics=args.n_topics)
    ...
    trainer = ReconstructionBERTTrainer(bert, len(vocab), len(markdown_vocab), args.markdown_emb_size, train_dataloader=train_data_loader, test_dataloader=test_data_loader,
                                        lr=args.lr, betas=(
        args.adam_beta1, args.adam_beta2), weight_decay=args.adam_weight_decay,
    ...
    for epoch in range(args.epochs):
        if trainer.last_epoch and epoch <= trainer.last_epoch:
            continue
