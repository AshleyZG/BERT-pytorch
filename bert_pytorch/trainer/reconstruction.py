...
class ReconstructionBERTTrainer:
    ...
    def __init__(self, bert: BERT, vocab_size: int,
                 train_dataloader: DataLoader, test_dataloader: DataLoader, neg_train_dataloader: DataLoader, neg_test_dataloader: DataLoader,
                 lr: float = 1e-4, betas=(0.9, 0.999), weight_decay: float = 0.01, warmup_steps=10000,
                 with_cuda: bool = True, cuda_devices=None, log_freq: int = 10, pad_index=0, loss_lambda=1, model_path=None, n_topics=50, weak_supervise=False):
        """
        ...
        self.model = VAE(bert, vocab_size, n_topics=n_topics,
                         weak_supervise=weak_supervise).to(self.device)
        if model_path:
        ...
        self.neg_train_data = neg_train_dataloader
        self.neg_test_data = neg_test_dataloader
        # pdb.set_trace()
    ...
    def train(self, epoch):
        ...
        return self.iteration(epoch, self.train_data, self.neg_train_data)

    def test(self, epoch):
        self.model.eval()
        with torch.no_grad():
            loss = self.iteration(epoch, self.test_data,
                                  self.neg_test_data, train=False)
        return loss
    ...
    def api(self):
        ...
        neg_data_loader = self.neg_test_data
        # Setting the tqdm progress bar
        data_iter = tqdm.tqdm(enumerate(data_loader),
                              # desc="EP_%s:%d" % (str_code, epoch),
                              total=len(data_loader),
                              bar_format="{l_bar}{r_bar}")
        neg_data_iter = tqdm.tqdm(enumerate(neg_data_loader),
                                  # desc="EP_%s:%d" % (str_code, epoch),
                                  total=len(neg_data_loader),
                                  bar_format="{l_bar}{r_bar}")
        avg_loss = 0.0
        total_correct = 0
        total_element = 0
    ...
    def iteration(self, epoch, data_loader, neg_data_loader, train=True):
        """
